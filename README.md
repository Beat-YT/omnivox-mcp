...
| `BROWSER_SLEEP` | `false` | When `true`, the browser closes after 5 minutes of inactivity and relaunches on the next request. Saves memory at the cost of a cold-start delay. Not recommended for Docker setups |
...
| `ENABLE_EXTERNAL_SERVICE_LINKS` | `false` | When `true` or `1`, enables `get-service-link` tool which hands pre-authenticated links to web-only Omnivox services (lockers, advisor appointments, progression chart, ...). These link logs into the full web portal, which is why it is off by default. Enable at your own risk. With `MCP_SERVER_URL` set the tool returns a short link; otherwise it returns the raw single-use URL. |
| `DISABLE_LOGGER` | `3000` | Disables server.log file writing in data folder |
...
- **[The web handoff](docs/web-handoff.md)**: how a portal-only module in the manifest turns into the pre-authenticated URL the app opens in a native window, layer by layer.
